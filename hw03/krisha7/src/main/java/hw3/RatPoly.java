...
public final class RatPoly {
    ...
    public RatPoly(int c, int e) {
    	...
    	if (c != 0) {
            ...
            for (int i = 0; i < e; i++) { //empty initializer 
            	coeffs[i] = new RatNum(0);
            }
            coeffs[e] = new RatNum(c);
    	} else { //zero degree case
    		degree = 0;
    		coeffs = new RatNum[0];
    	}
    ...
    public RatNum getCoeff(int pow) {
    	...
    	if (pow < 0 || pow >= coeffs.length) {
    		...
    		return coeffs[pow]; //returns correct coefficient for respective power
    	}
    }
    ...
    private static void scaleCoeff(RatNum[] arr, RatNum scalar) {
    	...
    	if (scalar != null) {
    		for (int i = 0; i < arr.length; i++) {
    			arr[i] = arr[i].mul(scalar); //multiplies each value of array by scalar
    		}
    	} else {
    		arr = new RatNum[0];
    	}
    ...
    public RatPoly negate() {
    	...
    	if (this.equals(ZERO)) {
            ...
            for (int i = 0; i < degree + 1; i++) {
            	arr_temp[i] = coeffs[i].negate(); //negates coefficient values 
            }
    ...
    public RatPoly add(RatPoly p) {
    	...
    	if (this.equals(ZERO) && p.equals(ZERO)) {
        	...
        	if (degree <= p.degree) {
        		small = coeffs.length;
        		this_p = 1;
        		arr = new RatNum[p.coeffs.length];
        	} else {
        		small = p.coeffs.length;
        		this_p = 0;
        		arr = new RatNum[coeffs.length];
        	}
        	
        	for (int i = 0; i < arr.length; i++) { //adds respective coeffiecients 
        		if (i >= small) {
        			if (this_p == 0) {
        				arr[i] = coeffs[i];
        			} else {
        			...
        			arr[i] = coeffs[i].add(p.coeffs[i]);
        		}
        	}
        	
        	if(arr[arr.length-1].equals(new RatNum(0))) {
				return ZERO;
        	}
        	return new RatPoly(arr);
        }
    ...
    public RatPoly mul(RatPoly p) {
    	...
    	if (this.equals(ZERO) || p.equals(ZERO)) {
        	...
        	RatNum [] arr = new RatNum[coeffs.length + p.coeffs.length - 1];
        	for (int i = 0; i < arr.length; i++) {
        		arr[i] = new RatNum(0);
        	}
        	
        	for(int i = 0; i < coeffs.length; i++) {
        		for (int j = 0; j < p.coeffs.length; j++) { //adds and multiplies coefficients for new array
        			arr[i+j] = arr[i+j].add(coeffs[i].mul(p.coeffs[j]));
        		}
        	}
    ...
    public RatPoly div(RatPoly p) {
    	...
    	if (p.isNaN() || this.isNaN() || p.equals(ZERO)) {
            return RatPoly.NaN;
        } else if (this.coeffs.length < p.coeffs.length || this.equals(ZERO)) {
            return RatPoly.ZERO;
        } else {
        	...
        	for (int i = coeffs.length - p.coeffs.length; i >= 0 && rem.length >= p.coeffs.length && rem.length > 0; i--, j++) {
        		result[i] = rem[rem.length - j].div(p.coeffs[p.coeffs.length - 1]); //divides p coefficients from result array 
    ...
    public double eval(double d) {
    	...
    	if (this.isNaN()) {
        	...
        	for (int i = coeffs.length; i > 0; i--) {
        		num = num * d + coeffs[i-1].doubleValue();
        	}
    ...
    public RatPoly differentiate() {
    	...
    	if (this.isNaN()){
        	...
        	RatNum[] arr = new RatNum[coeffs.length-1];
        	
        	for (int i = 0; i < arr.length; i++) {
        		arr[i] = coeffs[i+1].mul(new RatNum(i+1));
        	}
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
    	...
    	if(integrationConstant.isNaN() || this.isNaN()){
        	...
        	RatNum[] num = new RatNum[coeffs.length+1];
        	num[0] = integrationConstant;
        	
        	for (int i = num.length - 1; i > 0; i--) {
        		num[i] = coeffs[i-1].div(new RatNum(i));
        	}
        	
        	if(num[num.length-1].equals(new RatNum(0))) {
				return ZERO;
        	}
    ...
    public double integrate(double lowerBound, double upperBound) {
        // TODO: Fill in this method, then remove the RuntimeException
    	if (Double.isNaN(lowerBound) || this.isNaN() || Double.isNaN(upperBound)) {
    		return RatNum.NaN.doubleValue();
    	} else {
    		RatPoly num = antiDifferentiate(new RatNum(1));
    		return num.eval(upperBound) - num.eval(lowerBound); //evaluates at upper and lower bounds takes difference 
    	}
    }
